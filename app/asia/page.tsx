...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title, description,
    alternates: { canonical: `${BASE_URL}/asia` },
    openGraph: { title, description, type: "website", url: `${BASE_URL}/asia`, siteName: "HalfSpace",
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto min-h-[60vh] max-w-7xl px-4 py-12">
        <div className="mb-8">
          ...
          <p className="mt-4 max-w-2xl text-lg text-muted-foreground">Latest news from Asian football.</p>
        </div>
        <div className="mb-12">
          <h2 className="mb-6 text-2xl font-bold uppercase tracking-tight text-foreground" style={{ fontFamily: "var(--font-oswald)" }}>
            Latest Asia News
          </h2>
          <GroupArticleGridServer groupKey="asia" title="Asia" />
        </div>
        <div>
          <h2 className="mb-6 text-2xl font-bold uppercase tracking-tight text-foreground" style={{ fontFamily: "var(--font-oswald)" }}>
            Competitions
